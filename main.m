%driver code
function main(vals_size)
    blueprint.value = 0;
    blueprint.is_assigned = false;
    blueprint.expression = [];
    blueprint.is_dependent = false;
...
%         end
        ...
        vals(i_curr ,j_curr).is_dependent = is_expression_dependent(expr);
...
%6
function action_show(cmd ,vals ,curr)
    index = cmd(6:end);
    if numel(index) == 0
        index = curr;
    end
...
%handles events
function [new_vals] = event_handler(vals)
    for i=1:numel(vals)
        if vals(i).is_dependent
            vals(i).value = evaluate_expression(vals(i).expression ,vals);
        end
    end
    new_vals = vals;
...
function [value] = evaluate_expression(expr ,vals)
    corrected_expr = [];
    i = 1;
    while i <= size(expr ,2) %Translates user input command to evaluate-function valid command
...
function [type] = is_expression_dependent(expr)
    i=1;
    while i <= size(expr ,2)
        if isletter(expr(i))
           for j=i+1:size(expr ,2)
               if isletter(expr(j))
                    continue;
               elseif ~isnan(str2double(expr(j)))
                    type = true;
                    return;
               else
                   i = j;
...
function [new_index] = translate_index(index ,vals)
    ...
    if indeces(1).i
        ...
        %sorted_indeces = sort([indeces.k]);
        %indeces(1).k = sorted_indeces(1);
        %indeces(2).k = sorted_indeces(2);
        sorted_i = sort([indeces.i]);
        indeces(1).i = sorted_i(1);
        indeces(2).i = sorted_i(2);
        sorted_j = sort([indeces.j]);
        indeces(1).j = sorted_j(1);
        indeces(2).j = sorted_j(2);

        new_index = [num2str(indeces(1).i) ,':' ,num2str(indeces(2).i) ,...
                    ' ,' ,...
                    num2str(indeces(1).j) ,':' ,num2str(indeces(2).j)
                    ];
        %new_index = [num2str(indeces(1).k) ,':' ,num2str(indeces(2).k)];
    else
        new_index = [num2str(indeces(1).i) ,' ,' ,num2str(indeces(2).j)];
        %new_index = num2str(indeces(2).k);
    end

    new_index = ['reshape([vals(', new_index, ').value] ,1 ,[])']; %usable index in eval function
